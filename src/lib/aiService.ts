...
// 智谱API基础配置
const ZHIPU_API_KEY = "3908a4b9a43a4ced97eb2c932a8f39a7.vcHYXmk3qs5FhxJK";
...
/**
 * 调用智谱API进行图片识别 - 先提取图片中的文字，再根据工具类型进行结构化识别
...
export async function recognizeImageWithZhipu(imageBase64: string, toolType: 'textQuote' | 'fullContainerQuote' | 'calculator' | 'inquiryOrganizer' = 'calculator'): Promise<any> {
  try {
    // 1. 首先使用OCR功能提取图片中的文字
    const ocrResponse = await fetch(`${ZHIPU_API_BASE_URL}/images/recognize`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${ZHIPU_API_KEY}`
      ...
      body: JSON.stringify({
        model: "glm-4v",
        prompt: "请提取这张图片中的所有文字内容，不要添加任何解释，只返回识别到的文本",
        images: [imageBase64]
      })
    });

    if (!ocrResponse.ok) {
      throw new Error(`图片OCR识别失败: ${ocrResponse.status}`);
    }

    const ocrData = await ocrResponse.json();
    const extractedText = ocrData.content || "";

    if (!extractedText) {
      throw new Error("未从图片中识别到文字内容");
    }

    // 2. 使用提取的文字调用现有的文本识别函数进行结构化处理
    return await recognizeTextWithZhipu(extractedText, toolType);
  } catch (error) {
    console.error("图片识别错误:", error);
    // 在实际环境中，这可能是由于API限制或不支持图片识别，我们提供一个模拟结果
    return {
      // 返回一个根据工具类型生成的模拟识别结果
      ...getMockImageRecognitionResult(toolType),
      isMock: true,
      rawText: "这是一个模拟的图片识别结果，因为实际的图片识别API调用失败"
    };
  }
}
